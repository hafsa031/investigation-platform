...
from app.ioc.schemas import AnalyzeResponse, IOCAnalyzeRequest, IOCRecord
...
try:  # extractor (real fn: extract_all(text) -> [{type,raw,start,end}])
    try:
        from app.ioc.extractor import extract_all as _extract_all  # type: ignore
    except Exception:
...
try:  # normalize (real fn: normalize(value, ioc_type))
    try:
        from app.ioc.normalize import normalize as _normalize_real  # type: ignore
    except Exception:
...
try:  # validate (real fns: is_valid_ip/domain/email/hash in validate.py)
    try:
        from app.ioc import validate as _validate_mod  # type: ignore
    except Exception:
...
try:  # allowlist (real fn: validate.is_allowlisted(value, allowlist=None))
    try:
        from app.ioc.validate import is_allowlisted as _allowlisted_real  # type: ignore
    except Exception:
...
try:  # scoring
    try:
        from app.ioc.scoring import score_ioc as _score_fn  # type: ignore
    except Exception:
...
try:  # mitre_map (exists in-repo)
    try:
        from app.ioc.mitre_map import map_ioc as _mitre_fn  # type: ignore
    except Exception:
...
def analyze(request: IOCAnalyzeRequest) -> AnalyzeResponse:
    ...
    for hit in raw_hits:
        ...
        try:
            from app.ioc.extractor import refang_text as _refang  # type: ignore
        except Exception:
            try:
    ...
    if use_ai and records:
        try:
            from app.ioc import groq_rerank as _groq  # type: ignore[import]
            from app.ioc import quota as _quota  # type: ignore[import]
        except Exception:
